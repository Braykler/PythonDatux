...
def IngestDataProducts(app:App):
    ...
    dataCity=GetDataSourceCity()
    CreateTableCity(conn)
    InsertDataCity(bd,dataCity)
...
def GetDataSourcePais():
    pathData="/workspaces/PythonDatux/proyectos/files/datafuente.xls"
...
def GetDatasourceOrders(conn):
    ...
    df_orders=df[['Order ID','Postal Code','Product ID','Sales','Quantity','Discount','Profit','City','Shipping Cost','Order Priority']].dropna().drop_duplicates()
    ...
    df_newOrders=df_newOrders[['Order ID','Postal Code','id','Sales','Quantity','Discount','Profit','City','Shipping Cost','Order Priority']]
    list_tuples=[tuple(x) for x in df_newOrders.to_records(index=False)]
    return list_tuples


def GetDataSourceCity():
    pathData="/workspaces/PythonDatux/proyectos/files/datafuente.xls"
    df=pd.read_excel(pathData,sheet_name="Orders")
    print(df.shape)
    print(df.keys())
    ...
    print(df_City.shape)
    City_tuples = [(City,) for City in df_City] 
    
    return City_tuples

def CreateTableCity(conn:Connection):
    city=City()
    city.create_table(conn)

def InsertDataCity(bd:Database,data):
    bd.insert_many('CITY',['name'],data)
...
def insertManyVentas(bd:Database,data):
    bd.insert_many('VENTAS',['order_id','postal_code','product_id','sales_amount','quantity','discount','profit','city', 'shipping_cost','order_priority'],data)